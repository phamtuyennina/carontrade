<?php 
	...
	if(!empty($_GET['id_kieudang'])){
		$arr_hang=explode('.',$_GET['id_kieudang']);
		$arr_hang1=implode(',',$arr_hang);
		$d->reset();
		$sql="select ten$lang as ten,tenkhongdau,id,photo from #_product_danhmuc where hienthi=1 and id_kieudang in(".$arr_hang1.") and type='mau-xe' order by stt,id desc";
		$d->query($sql);
		$mauxe=$d->result_array();	
	}elseif(!empty($_GET['id_hang'])){
		$arr_hang=explode('.',$_GET['id_hang']);
		$arr_hang1=implode(',',$arr_hang);
		$d->reset();
		$sql="select ten$lang as ten,tenkhongdau,id,photo from #_product_danhmuc where hienthi=1 and id_thuonghieu in(".$arr_hang1.") and type='mau-xe' order by stt,id desc";
		$d->query($sql);
		$mauxe=$d->result_array();	
	}else{
		$d->reset();
		$sql="select ten$lang as ten,tenkhongdau,id,photo from #_product_danhmuc where hienthi=1 and type='mau-xe' order by stt,id desc";
		$d->query($sql);
		$mauxe=$d->result_array();	
