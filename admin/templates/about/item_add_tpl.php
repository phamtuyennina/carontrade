...
<form name="supplier" id="validate" class="form" action="index.php?com=about&act=save&type=<?=$_REQUEST['type']?>" method="post" enctype="multipart/form-data">
	<div class="widget">
        ...
        <ul class="tabs">
           <?php if($_REQUEST['type']!='lienhe' && $_REQUEST['type']!='footer' && $_REQUEST['type']!='dangky' && $_REQUEST['type']!='dinhgiaxe' && $_REQUEST['type']!='motacapdo' && $_REQUEST['type']!='link'){ ?>
           <li>
	...
	<?php if($_REQUEST['type']!='lienhe' && $_REQUEST['type']!='footer' && $_REQUEST['type']!='dangky' && $_REQUEST['type']!='dinhgiaxe' && $_REQUEST['type']!='motacapdo' && $_REQUEST['type']!='link'){ ?>
     ...
     <?php if($_REQUEST['type']=='about' or $_REQUEST['type']=='hop-tac' or $_REQUEST['type']=='tuyen-dung'){ ?>
		<div class="formRow">
            ...
            <input type="file" id="file" name="file" /><img src="./images/question-button.png" alt="Upload hình" class="icon_question tipS" original-title="Tải hình đại diện cho sản phẩm (ảnh JPEG, GIF , JPG , PNG)">
              <div class="note"> Height:300px | Width:400px  <?=_format_duoihinh_l?> </div>                 
            </div>
            <div class="clear"></div>
        </div>
     <?php } ?>
      ...
      <?php foreach ($config['lang'] as $key => $value) { ?>
       ...
       <div id="content_lang_<?=$key?>" class="tab_content <?php if($_REQUEST['type']!='lienhe' && $_REQUEST['type']!='footer' && $value=='' && $_REQUEST['type']!='dangky' && $_REQUEST['type']!='dinhgiaxe' && $_REQUEST['type']!='motacapdo' && $_REQUEST['type']!='link')echo 'info' ?>">   
         <?php if($_REQUEST['type']=='about'  or $_REQUEST['type']=='hop-tac' or $_REQUEST['type']=='tuyen-dung'){ ?>     
