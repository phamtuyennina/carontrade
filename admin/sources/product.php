...
function get_danhmucs(){
	...
	{
		$where.=" and id_thuonghieu=".$_REQUEST['id_thuonghieu']."";
	}
	if((int)$_REQUEST['id_kieudang']!='')
	{
		$where.=" and id_kieudang=".$_REQUEST['id_kieudang']."";
	}	
	if($_REQUEST['key']!='')
	{
		$where.=" and ten like '%".$_REQUEST['key']."%'";
...
function save_danhmuc(){
	...
	if($id){		
		...
		if($banner = upload_image("file1", _format_duoihinh, _upload_sanpham,$file_name1)){
			$data['banner'] = $banner;	
			$d->setTable('product_danhmuc');
			$d->setWhere('id', $id);
			$d->select();
		...
		$data['id_kieudang'] = (int)$_POST['id_kieudang'];
		$data['tenkhongdau'] = changeTitle($_POST['ten']);
		$data['stt'] = $_POST['stt'];
		$data['hienthi'] = isset($_POST['hienthi']) ? 1 : 0;
		$data['ngaysua'] = time();
		...
		if($_REQUEST['type']!='mau-xe'){
			$data['id_thuonghieu']=implode(',',$_POST['id_thuonghieu']);
		}else{
