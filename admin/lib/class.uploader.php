...
class Uploader {
	...
	private function prepareFiles(){
        ...
        if($validate){
            ...
            $field_name=array_values($field['name']);
			$field_Field_Type=$field['Field_Type'];
			$field_tmp_name=array_values($field['tmp_name']);
			$field_type=array_values($field['type']);
			$field_size=array_values($field['size']);
			$field_error=array_values($field['error']);
			
            for($i = 0; $i < count($field_name); $i++){
                
                $metas = array();
                
                if($field_Field_Type == 'input'){
                    $tmp_name = $field_tmp_name[$i];
					
                }elseif($field_Field_Type == 'link'){
                    $link = $this->downloadFile($field_name[0], false, true);
                    
                    $tmp_name = $field_name[0];
                    $field_name[0] = pathinfo($field_name[0], PATHINFO_BASENAME);
                    $field_type[0] = $link['type'];
                    $field_size[0] = $link['size'];
                    $field_error[0] = 0;
                }
                
                $metas['extension'] = substr(strrchr(strtolower($field_name[$i]), "."),1);
                $metas['type'] = preg_split('[/]', $field_size[$i]);
                $metas['extension'] = $field_Field_Type == 'link' && empty($metas['extension']) ? $metas['type'][1] : $metas['extension'];
                $metas['old_name'] = substr($field_name[$i], 0, -(strlen(strrchr($field_name[$i], $metas['extension']))+1));
                $metas['size'] = $field_size[$i];
                $metas['size2'] = $this->formatSize($metas['size']);
                $metas['name'] =$metas['old_name'].time().(!empty($metas['extension']) ? "." . $metas['extension'] : "");
                $metas['file'] = $this->options['uploadDir'] . $metas['name'];
                $metas['replaced'] = file_exists($metas['file']);
                $metas['date'] = date('r');
                
                if(!in_array($field_name[$i], $removed_files) && $this->validate(array_merge($metas, array('index'=>$i, 'tmp'=>$tmp_name))) && $this->uploadFile($tmp_name, $metas['file'])){
            ...
            $this->data['isSuccess'] = count($field_name) - count($removed_files) == count($files);
